...
mtcars %>% 
  ...
  geom_pcp(aes(color = vs), boxwidth = 0.2) +
  geom_pcp_box(boxwidth = 0.2) +
  geom_pcp_band(boxwidth = 0.2, fill = "blue", alpha = 0.2)
```

Sometimes smaller bands are merged to a larger band, which is meaningful in showing a tidy plot, but will make it more difficult to color the bands. So we provide an argument to control this behavior.
