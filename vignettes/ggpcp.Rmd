...
When you are dealing with a relatively large data set, the overdrawing of lines from `geom_pcp` can be a problem. Note that the lines from `geom_pcp` are drawn according to the order they shown in your data set. So when the order changes in your data set the overdrawing pattern will also change. It can be messy and really annoying. We provided some overdrawing methods to solve that, by setting the argument `arrange` to:
