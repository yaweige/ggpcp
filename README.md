...
```r
flea %>%
  ggplot(aes(colour = species, vars=product(aede1, aede2, aede1, aede3))) + 
  geom_pcp2()
```
...
## }
## debug: if (!is.null(self$mapping$vars)) aes_vars <- tidyselect::vars_select(tbl_vars(data), 
##     !!!rlang::eval_tidy(self$mapping$vars))
## debug: if (!is.null(plot$mapping$vars)) aes_vars <- tidyselect::vars_select(tbl_vars(data), 
##     !!!rlang::eval_tidy(plot$mapping$vars))
## debug: aes_vars <- tidyselect::vars_select(tbl_vars(data), !!!rlang::eval_tidy(plot$mapping$vars))
## debug: var_x <- paste0("x__", 1:length(aes_vars), "__", as.character(aes_vars))
...
## }
## debug: self$mapping[[var_x[i]]] <- as.symbol(eval(aes_vars[[i]]))
## debug: self$mapping[[var_x[i]]] <- as.symbol(eval(aes_vars[[i]]))
## debug: self$mapping[[var_x[i]]] <- as.symbol(eval(aes_vars[[i]]))
...
## debug: evaled
## Called from: f(...)
## debug: idx <- grep("x__", names(data))
## debug: names(data) <- gsub("x__[0-9]+__", "", names(data))
## debug: data <- data.frame(data, stringsAsFactors = TRUE)
## debug: data <- gather_pcp2(data, idx)
## debug: data <- transform_pcp(data, method = params$method)
## debug: data
## Called from: f(...)
...
flea %>%
  ...
  geom_pcp2(aes(vars = product(starts_with("aede"), 1:4)), 
            method = "uniminmax")
```

```
## Warning: Ignoring unknown aesthetics: vars
```
...
## debug: aes_vars <- plot$mapping$vars
## debug: if (is.null(aes_vars)) aes_vars <- self$mapping$vars
## debug: aes_vars <- self$mapping$vars
## debug: if (!is.null(aes_vars)) {
##     if (!is.null(self$mapping$vars)) 
##         aes_vars <- tidyselect::vars_select(tbl_vars(data), !!!rlang::eval_tidy(self$mapping$vars))
##     if (!is.null(plot$mapping$vars)) 
...
## debug: if (!is.null(self$mapping$vars)) aes_vars <- tidyselect::vars_select(tbl_vars(data), 
##     !!!rlang::eval_tidy(self$mapping$vars))
## debug: aes_vars <- tidyselect::vars_select(tbl_vars(data), !!!rlang::eval_tidy(self$mapping$vars))
## debug: if (!is.null(plot$mapping$vars)) aes_vars <- tidyselect::vars_select(tbl_vars(data), 
##     !!!rlang::eval_tidy(plot$mapping$vars))
## debug: var_x <- paste0("x__", 1:length(aes_vars), "__", as.character(aes_vars))
## debug: if (is.null(self$mapping)) self$mapping <- aes()
...
## debug: if (!is.null(self$mapping$vars)) self$mapping <- self$mapping[-grep("vars", 
##     names(self$mapping))]
## debug: self$mapping <- self$mapping[-grep("vars", names(self$mapping))]
## debug: data
## Called from: f(..., self = self)
## debug: if (!is.null(plot$mapping$vars)) plot$mapping <- plot$mapping[-grep("vars", 
##     names(plot$mapping))]
